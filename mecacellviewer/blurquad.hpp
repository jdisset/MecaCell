...
public:
	...
	void draw(GLuint tex, int amount, const QSize &s, const QRect &r) {
		...
		shader.setUniformValue(shader.uniformLocation("xRatio"), (float)r.width() / (float)s.width());
		for (int i = 0; i < amount; ++i) {
			if (i == 0) GL->glBindTexture(GL_TEXTURE_2D, tex);
			shader.setUniformValue(shader.uniformLocation("dir"), QVector2D(1.0, 0));
			GL->glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
			shader.setUniformValue(shader.uniformLocation("dir"), QVector2D(0.70710678118, 0.70710678118));
			GL->glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
			shader.setUniformValue(shader.uniformLocation("dir"), QVector2D(-0.70710678118, 0.70710678118));
			GL->glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
		...
		quad.vao.release();
		shader.release();
		QOpenGLFramebufferObject::blitFramebuffer(fboA.get(), r, fboB.get(), r, GL_COLOR_BUFFER_BIT, GL_LINEAR);

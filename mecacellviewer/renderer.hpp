...
class Renderer : public SignalSlotRenderer {
 ...
 private:
	...
	// draw methods
	struct VisualObject {
		std::function<void(R *)> draw;
		bool checkable = true;
	};

	QMap<QString, VisualObject> drawMethods =
	{ {"skybox", {[&]() { skybox.draw(view, projection, camera); }}},

		{"cells",
		 {[&]() {
		   cells.drawMode = plain;
		   cells.draw(scenario.getWorld().cells, view, projection, camera.getViewVector(),
		              camera.getPosition(), selectedCell);
		 }}},
		{"centers",
		 {[&]() {
...
virtual void paint() {
	...
	projection = camera.getProjectionMatrix((float)viewportSize.width() /
	...
	for (auto &d : orderedDrawList) {
		if (gc.contains(d) || ) {
			if (usersDrawMethods.contains(e)) {
				usersDrawMethods[d](this);
			} else if (drawMethods.contains(e)) {
				drawMethods[d]();
			}
		}
	}
	...
	if (gc.contains("centers")) {
		cells.drawMode = centers;
		cg.cells.draw(scenario.getWorld().cells, view, projection, camera.getViewVector(),
		              camera.getPosition(), selectedCell);
	}
	QOpenGLFramebufferObject::blitFramebuffer(
	    fsaaFBO.get(), QRect(QPoint(0, 0), viewportSize * screenCoef), finalFBO.get(),
	    QRect(QPoint(0, 0), viewportSize * FSAA_COEF * screenCoef),
...
virtual void initialize() {
	...
	ssaoFBO->bind();
	GL->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthTex,
	                           0);
	ssaoFBO->release();
	// scenario
}
...
public:
vector<string> orderedDrawList{{"skybox", "cells", "centers", "connections", "models",
                                "forces", "velocities", "cellGrid", "modelGrid"}};
...
Button < Renderer<Scenario, Plugins...> &getButton(const std::string &name) {
	return buttonMap.at(QString::fromStdString(name));
}
std::map<QString, Button<Renderer<Scenario, Plugins...>>> buttonMap;
void addButton(std::string name, std::string menu, std::string label,
               std::function<void(Renderer<Scenario, Plugins...> *)> onClicked) {
	Button < Renderer<Scenario, Plugins...> b(QString::fromStdString(name),
	                                          QString::fromStdString(menu),

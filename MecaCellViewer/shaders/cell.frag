...
void main(){
   ...
   tangentSpaceNormal = normalize(tangentSpaceNormal);
    ...
    vec3 normalColor = mixV3(vec3(1.0),vec3(0.0),max(0.0,min(1.0, diffuseCoefMap))) ; // éclairage caméra n&b avec normales perturbées
   ...
   /* vec3 final = reducedColor;*/

	final = superpose(final,whiteHalo,0.15);

	final = superpose(final,whiteHalo*vec4(normalColor,1.0),0.2);
   ...
   /*gl_FragColor = vec4(objectSpaceTangent,1.0) ;*/
   /*gl_FragColor.rgb = texture2D(texture,texCoordVar).rgb;*/
   /*[>gl_FragColor.rgb = gl_FragColor.rgb*gl_FragColor.rgb*gl_FragColor.rgb;<]*/
   /*gl_FragColor.a=1.0;*/
